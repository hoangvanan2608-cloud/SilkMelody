...
public class TalkableNPC : InRangeInteractable
{
    ...
    {
        bool pressTalk = inputMaster.Gameplay.Attack.WasPressedThisFrame();
        if (playerInRange && pressTalk && !player.inMenu && !isTalking) 
        { 
            isTalking = true;
            StopAllCoroutines();
    ...
    private IEnumerator Talking()
    {
        ...
        {
            if (
    inputMaster.Dialogue.Continue.WasPressedThisFrame()
    || inputMaster.Gameplay.Attack.WasPressedThisFrame()
)
            {
                dialogueId += 1;
