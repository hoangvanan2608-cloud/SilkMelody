...
public class BossBatterAI : MonoBehaviour
{
    ...
    public enum Moveset
    { whiteBall, attack, charge, jumpSlam, summon }
    ...
    private bool PlayerInRange()
    {
        return Vector2.Distance(
            transform.position,
            player.transform.position) <= detectRange;
    }
    ...
    {
        if (boulderSpawnZoneLeft == null ||
        boulderSpawnZoneRight == null)
        {
            Debug.LogError("Missing Boulder Spawn Zones!");
            return;
        }
        ...
        for (int i = 0; i < numBoulderSpawn; i++)
        {
            float randomX = Random.Range(
                boulderSpawnZoneLeft.position.x,
                boulderSpawnZoneRight.position.x);

            float randomY = Random.Range(
                boulderSpawnZoneLeft.position.y,
                boulderSpawnZoneRight.position.y);
