...
public class Player : MonoBehaviour
{
    ...
    [HideInInspector] public bool mobileAttackTriggered;
    private float mobileHorInput;
    private float mobileVerInput;

    private bool mobileJumpHeld;
    public bool mobileJumpPressed;
    private bool mobileJumpReleased;
    public bool mobileJumpTriggered;
    public bool mobileAttackPressed;
    private bool mobileDashPressed;
    private bool mobileParryPressed;
    private bool mobileSilkSkillPressed;
    private bool mobileShootPressed;
    #endregion Variables
    ...
    private void HandleMovement()
    {
        ...
        Vector2 direction =
            inputMaster.Gameplay.Movement.ReadValue<Vector2>();
        ...
        if (useMobileInput)
        {
            if (Mathf.Abs(mobileHorInput) > 0.01f)
                horInput = mobileHorInput;

            if (Mathf.Abs(mobileVerInput) > 0.01f)
                verInput = mobileVerInput;
        }
        ...
        rb.linearVelocity = new Vector2(
            horInput * playerStat.moveSpeed * speedModifier,
            rb.linearVelocity.y);
    }
    private void HandleJump()
    {
        InputAction jumpAction = inputMaster.Gameplay.Jump;
        if (jumpAction.WasPressedThisFrame() || mobileJumpPressed)
        {
            // Jump from ground
        ...
        if ((jumpAction.IsPressed() || mobileJumpHeld) && isJumping)
        {
            if (jumpTimer > 0)
            {
        ...
        if((jumpAction.WasReleasedThisFrame() || mobileJumpReleased) && isJumping)
        {
            jumpTimer = 0f;
            isJumping = false;
    ...
    private void HandleDash()
    {
        ...
        if ((dashAction.WasPressedThisFrame() || mobileDashPressed) && !isDashing && dashCount > 0)
        {
            dashCount--;
            dustPE.Play();
    ...
    private void HandleAttack()
    {
        ...
        if ((attackAction.WasPressedThisFrame() || mobileAttackPressed)
    && attackTimer > playerStat.attackCooldown
    && !inAttack
    && !resting)
        {
            // Up attack
    ...
    private void HandleParry()
    {
        ...
        if ((parryAction.WasPressedThisFrame() || mobileParryPressed) && !inAttack && isGrounded)
        {
    ...
    public void HandleSilkSkill()
    {
        ...
        if ((silkAction.WasPressedThisFrame() || mobileSilkSkillPressed) && !inAttack)
        {
            // Gossamer
    ...
    public void AttackDealDamage(bool isUpAttack)
    {
        ...
        if (isFacingLeft)
        {
            spawnPos.x = transform.position.x - Mathf.Abs(attackPos.localPosition.x);
        }
        else
        {
            spawnPos.x = transform.position.x + Mathf.Abs(attackPos.localPosition.x);
    ...
    #region Mobile Button Functions
    ...
    public void MobileMoveUpDown()
    {
        mobileVerInput = 1f;
    }

    public void MobileMoveDownDown()
    {
        mobileVerInput = -1f;
    }

    public void MobileMoveHorizontalUp()
    {
        mobileHorInput = 0f;
    }

    public void MobileMoveVerticalUp()
    {
        mobileVerInput = 0f;
    }
    ...
    {
        RestChair chair = FindObjectOfType<RestChair>();

        if (chair != null && chair.playerInRange)
        {
            chair.TryInteract();
            return;
        }
    ...
    {
        mobileParryPressed = true;
    }

    public void MobileHeal()
    {
        mobileSilkSkillPressed = true;
    }
    ...
    private void ResetMobileButtonInput()
    {
        mobileJumpPressed = false;
        mobileJumpReleased = false;
        mobileAttackPressed = false;
        mobileDashPressed = false;
        mobileParryPressed = false;
        mobileSilkSkillPressed = false;
        mobileShootPressed = false;
        mobileAttackTriggered = false;
        mobileJumpTriggered = false;
    }
